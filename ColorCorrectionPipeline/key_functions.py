...
__all__ = [
    ...
    "do_color_adaptation",
    "get_color_patch_size",
    "extract_neutral_patches",
    "poly_func",
    "extrapolate_if_saturated_mat",
    "estimate_gamma_profile",
    "wb_correction",  #'poly_func_cupy',
    "compute_diag",
    "color_correction_1",
    "color_correction",
    "scatter_RGB",
    ...
    "adapt_chart",
    "arrange_metrics",
    "get_json_file",  #'generate_powers_with_combinations',
...
class metrics:
    def __init__(self, deltaE, mse, mae):
        ...
        self.Data = pd.DataFrame(columns=["DeltaE", "MSE", "MAE"], data=data)
...
class CustomNN(BaseEstimator, RegressorMixin):
    ...
    def _build_model(self):
        ...
        (
            log_(f"Using device: '{self.device}'", "purple", "italic")
            if self.verbose
            else None
        )
        layers = []
        ...
        for hidden_dim in self.hidden_layers:
            ...
            if self.use_batch_norm:
                layers.append(
                    nn.BatchNorm1d(hidden_dim, eps=self.tol)
                )  # Batch normalization
    ...
    def fit(self, X, y):
        ...
        optimizer = optimizer_types[self.optim_type](
            self.model.parameters(), lr=self.learning_rate
        )

        scheduler = (
            optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode="min", factor=0.90, patience=int(0.15 * self.patience)
            )
        ...
        for epoch in range(self.max_epochs):
            ...
            if self.verbose and epoch % interval == 0:
                log_(
                    f"Epoch {epoch + 1}/{self.max_epochs}, Train Loss: {train_loss:.8f}, Val Loss: {val_loss:.8f}, Learning Rate: {optimizer.param_groups[0]['lr']:.6f}",
                    "light_blue",
                    "italic",
                )
            ...
            if val_loss < best_val_loss and not model_over_fit:
                log_(
                    f"Saving model with 'validation loss' = {val_loss:.8f}, @ epoch {epoch + 1}",
                    "green",
                    "italic",
                )
            ...
            if self.early_stopping and epochs_no_improve >= self.patience:
                if self.verbose:
                    log_(
                        f"Early stopping triggered @ epoch {epoch + 1}",
                        "yellow",
                        "italic",
                    )
                break
        ...
        with torch.no_grad():
            try:
                ...
                while not success:
                    try:
                        log_(
                            f"Trying to break prediction into batches of {batch_size}",
                            "yellow",
                            "italic",
                        )
                        ...
                        if "out of memory" in str(e):
                            log_(
                                f"Out of memory error with batch size {batch_size}: {e}",
                                "red",
                                "italic",
                            )
                            batch_size *= 2
...
def check_memory():
    with memory_lock:
        ...
        if "torch" in globals() and torch.cuda.is_available():
            ...
            log_(
                f"Used GPU memory: {torch.cuda.memory_usage(0):.2f}%",
                "light_blue",
                "italic",
            )
...
def get_color_patch_size(color_checker_img: np.ndarray) -> np.ndarray:
    ...
    min_area = (
        0.01 * color_checker_img.shape[0] * color_checker_img.shape[1]
    )  # 1% of image area
    ...
    for cnt in contours:
        ...
        if h > 0 and w > 0:
            area = w * h
            if (
                area > min_area and min(w / h, h / w) > 0.8
            ):  # Ratio close to 1 for squares
                dims.append((w, h))
...
def detect_refine_charts(detector, img, n_charts, params=CDP):
    ...
    if n_charts == 1:
        ...
        if (len_ - 1) < n_charts:
            log_(
                f"Detected {len(charts)-1} charts instead of {n_charts}.",
                "yellow",
                "italic",
                "warn",
            )
            log_(
                "Consider adjusting `cv2.mcc.DetectorParameters()` for improved results.",
                "yellow",
                "italic",
                "warn",
            )
...
# det_params =
def extract_color_charts(
    img: np.ndarray, n_charts: int = 1
...
def extract_color_chart_ex(
    ...
    # Input validation
    if npts < 1:
        log_(f"Warning: npts must be >= 1, got {npts}. Using npts=1", "yellow", "italic", "warn")
        npts = 1
    
    if img is None or img.size == 0:
        log_("Error: Invalid input image", "red", "bold", "error")
        return None, None
    
    if ref is None or ref.shape != (24, 3):
        log_(f"Error: Invalid reference shape. Expected (24, 3), got {ref.shape if ref is not None else None}", "red", "bold", "error")
        return None, None
    ...
    try:
        ...
        if n_patches != 24:
            log_(f"Warning: Expected 24 patches, got {n_patches}", "yellow", "italic", "warn")
        ...
        for patch_id, (mask, ref_color) in enumerate(zip(mask_list, ref)):
            try:
                ...
                if patch_pixels.shape[0] == 0:
                    log_(f"Warning: Empty patch {patch_id}, skipping", "yellow", "italic", "warn")
                    continue
                ...
                # Direct array assignment (FIXED: explicit repetition instead of broadcast_to)
                end_idx = current_idx + npts
                chart_result[current_idx:end_idx, :] = all_data
                ref_result[current_idx:end_idx, :] = ref_color  # Broadcasting
                
                current_idx = end_idx
                valid_patches += 1
                
            except Exception as e:
                log_(f"Warning: Error processing patch {patch_id}: {e}", "yellow", "italic", "warn")
                continue
        
        # Validate results
        if valid_patches == 0:
            log_("Error: No valid patches extracted", "red", "bold", "error")
            return None, None
        ...
        log_(f"Error in extract_color_chart_ex: {e}", "red", "bold", "error")
        import traceback
        traceback.print_exc()
        return None, None
...
def extract_neutral_patches(
    img: np.ndarray, return_one: bool = True, show: bool = False
):
    ...
    if return_one:
        ...
        src_charts, img_draw = extract_color_charts(
            img, n_charts=3, get_patch_size=True
        )

    if show:
        colour.plotting.plot_image(
            to_float64(img_draw[:, :, ::-1]), title=f"Charts found: {len(src_charts)}"
        )
...
def poly_func_torch(x: torch.Tensor, coeffs: torch.Tensor) -> torch.Tensor:
    result = torch.zeros_like(
        x
    )  # Initialize result as a tensor of zeros with the same shape as x
    for c in coeffs:
...
def predict_image(
    img: np.ndarray,
    coeffs: np.ndarray,
    ref_illuminant: np.ndarray = colour.CCS_ILLUMINANTS[
    ...
    if is_cuda:
        try:
            ...
            img_flat_gpu = torch.from_numpy(img_flat_L.copy()).to(
                device_, dtype=torch.float32
            )
            coeffs_gpu = torch.from_numpy(coeffs_copy).to(
                device_, dtype=torch.float32
...
def get_metrics(
    mat1: np.ndarray, mat2: np.ndarray, illuminant: np.ndarray, c_space: str = "srgb"
) -> np.ndarray:
...
def arrange_metrics(metrics_before, metrics_after, name=""):
    ...
    all_data_before.columns = [
        f"Before_{name}_" + col for col in all_data_before.columns
    ]
    ...
    Metrics_all = {"Summary": Metrics_summary, "All": All_data_pd}
...
def convert_to_lab(
    mat: np.ndarray,
    illuminant: np.ndarray = colour.CCS_ILLUMINANTS[
        "CIE 1931 2 Degree Standard Observer"
    ]["D65"],
    c_space: str = "xyz",
) -> np.ndarray:
...
def srgb_to_cielab_D50(
    srgb_mat: np.ndarray, srgb_mat_illuminant: np.ndarray
) -> np.ndarray:
...
def convert_to_LCHab(
    mat: np.ndarray, illuminant: np.ndarray, c_space: str = "xyz"
) -> np.ndarray:
    c_space_list = ["xyy", "lab", "xyz", "srgb"]
    c_space = match_keywords(c_space, c_space_list)
    ...
    if c_space == "xyy":
        ...
        return colour.Lab_to_LCHab(
            colour.XYZ_to_Lab(colour.sRGB_to_XYZ(mat), illuminant)
        )
    else:
        raise ValueError(f"Invalid colour space: {c_space}")
...
def extrapolate_if_saturated_mat(
    mat: np.ndarray, mat_ref: np.ndarray, n_proc: int = 4
) -> np.ndarray:
    """
...
def extrapolate_if_sat_image(img: np.ndarray, mat_ref: np.ndarray) -> np.ndarray:
    ...
    new_img_flat = (new_img_flat - new_img_flat.min()) / (
        new_img_flat.max() - new_img_flat.min()
    )
...
def generate_powers_with_combinations_torch(features, names, degree=1):
    ...
    for d in range(1, degree + 1):
        for comb in itertools.combinations_with_replacement(range(n_features), d):
            ...
            f_names.append(
                " ".join(f"{names[i]}^{comb.count(i)}" for i in unique_comb).replace(
                    "^1", ""
                )
            )
...
def process_in_chunks(RGB, feature_names, degree, n_chunk):
    ...
    for i in range(n_chunk):
        ...
        RGB_torch = (
            torch.from_numpy(RGB_chunk).pin_memory().to(device_, dtype=torch.float32)
        )

        # Compute polynomial features
        X_poly_, names = generate_powers_with_combinations_torch(
...
def estimate_chunk_size(RGB):
    ...
    if do_chunks:
        log_(
            f"Available GPU memory: {available_memory / 1024**3:.2f} GB is low",
            "yellow",
            "italic",
        )
...
def fit_model(det_p, ref_p, kwargs=None):
    ...
    if fit_method not in model_dict:
        raise ValueError(
            f'"{fit_method}" is not a valid model type. Available model types: {model_dict.keys()}'
        )
...
def get_illuminant_from(type_="", var=None, CMFS="CIE 1931 2 Degree Standard Observer"):
    ...
    if var is None:
        ...
        if type_ == "spectra":
            ...
            log_(
                f"Using Spectral Distribution, detected illuminant: x: {REF_ILLUMINANT[0]}, y: {REF_ILLUMINANT[1]}",
                "cyan",
                "italic",
            )
            ...
            log_(
                f"Using xy: x: {REF_ILLUMINANT[0]}, y: {REF_ILLUMINANT[1]}",
                "cyan",
                "italic",
            )
            ...
            log_(
                f"Computed from uv* chromaticity values: x: {REF_ILLUMINANT[0]}, y: {REF_ILLUMINANT[1]}",
                "cyan",
                "italic",
            )
            ...
            log_(
                f"Detected Temperature from neutral patches, Temperature: {T.astype(int)}K, \tx: {x}, \ty: {y}",
                "cyan",
                "italic",
            )
    ...
    if CRI is not None:
        ...
        if spectra is None:
            ...
            der = "(Derived from xy)"
        SSI = colour.spectral_similarity_index(spectra, d65_spectra_dist, False)
        log_(f"Current Illuminant CRI: {CRI}", "light_green", "italic")
        log_(f"D65 CRI: {CRI_d65}", "light_green", "italic")
        log_(
            f"Spectral{der} Similarity Index between Current Illuminant and D65: {SSI}",
            "green",
            "italic",
        )
...
def estimate_gamma_profile(
    ...
    eq = "F(x) = " + "+".join(
        f"{round(c, 2)}x^{i}" for i, c in enumerate(reversed(coeffs))
    )
    # print(eq)
...
def color_correction_1(
    ...
    if get_deltaE:
        ...
        m_a = get_metrics(
            ref_rgb, color_patches_corrected.values, ref_illuminant, "srgb"
        )
...
def color_correction(
    ...
    _, color_patches = extract_neutral_patches(
        img_bgr, show=show if n_samples == 1 else False
    )
    cp_values = color_patches.values
...
def plot_raincloud(
    ...
    x_label="Condition",
    y_label="DeltaE",
    title_name="DeltaE vs Condition",
    save_=None,
    show=True,
):
        ...
        if patch_names is not None:
            ...
            for name in patch_names:
                ...
                if key is not None:
                    ...
                    log_(
                        f'Patch "{name}" not found in color chart',
                        "yellow",
                        "italic",
                        "warn",
                    )
    ...
    if multiple > 1:
        ...
        if len(hex_values_) > len(condition_labels):
            ...
            # fill with #808080
            hex_values_ = np.append(
                hex_values_, ["#808080"] * (len(condition_labels) - len(hex_values_))
            )

    plot_data = pd.DataFrame({"Condition": condition_labels, "Metric": deltaE_values})
...
def get_stats(data: pd.DataFrame, condition_: str = "col") -> tuple:
    ...
    # Perform t-test
    try:
        t_stat, p_value, dof = sm.stats.ttest_ind(
            long_data[long_data["Condition"] == long_data["Condition"].unique()[0]][
                "DeltaE"
            ],
            long_data[long_data["Condition"] == long_data["Condition"].unique()[1]][
                "DeltaE"
            ],
        )
        # print("T-Test Results:\n", t_test_results)
        t_test_results = pd.DataFrame(
            {"T-Statistic": [t_stat], "Degrees of Freedom": [dof], "p-value": [p_value]}
        )
    except Exception as e:
    ...
    try:
        if long_data["Condition"].nunique() > 1:
            tukey_results = mc.pairwise_tukeyhsd(
                long_data["DeltaE"], long_data["Condition"]
            )
            tukey_df = pd.DataFrame(
                tukey_results.summary().data[1:],
...
def save_stats(
    ...
    (
        data.to_excel(
            os.path.join(
                save_path,
                "Stats",
                re.sub(r"[(),: ]", "_", title_name) + "_DATA_for_Stats.xlsx",
            )
        )
        if not data.empty
        else None
    ...
    (
        anova_results.to_excel(
            os.path.join(
                save_path, "Stats", re.sub(r"[(),: ]", "_", title_name) + "_ANOVA.xlsx"
            )
        )
    ...
    (
        tukey_df.to_excel(
            os.path.join(
                save_path, "Stats", re.sub(r"[(),: ]", "_", title_name) + "_TUKEY.xlsx"
            )
        )
    ...
    (
        t_test_df.to_excel(
            os.path.join(
                save_path, "Stats", re.sub(r"[(),: ]", "_", title_name) + "_T_TEST.xlsx"
            )
        )
...
def other_plots(data, save_path=None, title_name="", SHOW_=False):
    ...
    try:
        ...
        sns.heatmap(
            data[[col for col in data.columns if "_DeltaE" in col]].corr(),
            annot=True,
            cmap="coolwarm",
            robust=True,
        )
        ...
        if save_path is not None:
            plt.savefig(
                os.path.join(
                    save_path,
                    re.sub(r"[(),: ]", "_", title_name)
                    + "_Correlation_Heatmap_DeltaE.svg",
                ),
                dpi=1200,
            )
    ...
    try:
        ...
        sns.pairplot(
            data,
            hue="Method",
            vars=[col for col in data.columns if "_DeltaE" in col],
            height=3,
            aspect=1.2,
        )
        # plt.tight_layout()
