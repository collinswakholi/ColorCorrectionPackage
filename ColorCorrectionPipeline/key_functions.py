...
def extract_color_chart_ex(
    ...
    data = detect_colour_checkers_segmentation(
        img_crop_rgb, samples=int(1.2 * npts), additional_data=True
    )
    if len(data) < 1:
        try:
            log_(
                'Attempting to read color card again... with "n_samples =  1pt"',
                "light_yellow",
                "italic",
                "warn",
            )
    ...
    for mask, ref_color in zip(mask_list, ref):
        ...
        sampled_data = masked_data[
            np.random.choice(masked_data.shape[0], npts - 3, replace=False)
        ]
