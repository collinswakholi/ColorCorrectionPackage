...
from .ccp import (  # from ColorCorrectionPipeline import ColorCorrection, Config, to_float64
    ColorCorrection,
    Config,
)
...
SAVE_PATH = os.path.join(
    os.getcwd(), "results"
)  # path to save results if config.save is True
...
metrics, corrected_imgs, errors = cc.run(
    Image=img_rgb, White_Image=white_bgr, name_=img_name, config=config
)
