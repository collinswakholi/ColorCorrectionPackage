"""Run the bundled image pipeline with a batch-normalized NN CC config."""
...
OUT_DIR = Path("results") / "pipeline_cc_custom_nn_bn"
...
def main() -> None:
    ...
    metrics, images, error = pipeline.run(SAMPLE_IMAGE, WHITE_IMAGE, "custom_nn_bn", config)

    original_bgr = cv2.imread(SAMPLE_IMAGE)
    original_rgb = original_bgr[:, :, ::-1].astype(np.float64) / 255.0
    original_path = OUT_DIR / "custom_nn_bn_00_original.png"
    write_image(original_path, original_rgb)

    stage_images = {"original": str(original_path.resolve())}
    for index, stage in enumerate(STAGES, 1):
        key = f"custom_nn_bn_{stage}"
        if key in images:
            path = OUT_DIR / f"custom_nn_bn_{index:02d}_{stage.lower()}_sample.png"
